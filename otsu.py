...
def otsu_threshold(image: List[List[int]], manual_threshold: int = 0) -> Tuple[int, List[List[int]]]:
    ...
    O algoritmo busca o valor de limiar 't' (de 0 a 255) que maximiza a variância 
    entre-classes (between-class variance) ou, equivalentemente, minimiza a variância intra-classe.
    
    Maximiza sigma_b^2(t) = w_0(t) * w_1(t) * [mu_0(t) - mu_1(t)]^2
    Onde w são as probabilidades as classes e mu são as médias das classes.
    """
    height = len(image)
    width = len(image[0])
    ...
    if manual_threshold <= 0:
        ...
        # 2. Busca pelo limiar que maximiza a variância entre classes
        for t in range(256):
            weight_b += hist[t]
            if weight_b == 0:
                continue
...
def count_objects(binary: List[List[int]], min_area: int = 20) -> int:
    ...
    for y in range(height):
        for x in range(width):
            # Procura pixels brancos não visitados
            if binary[y][x] == 0 or visited[y][x]:
                continue
            
            # Novo objeto encontrado, mede o tamanho dele
            pixel_count = 0
            stack = [(y, x)]
            visited[y][x] = True
